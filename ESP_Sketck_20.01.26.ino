...
const String deviceId = "Robridge_BVS_06";
const String deviceName = "Robridge BVS 06";  // ✅ AI ENABLED - Contains "AI"
...
// ===== CHARGING DETECTION (TP4056/BQ24075) =====
#define CHARGING_PIN 27  // CHRG pin from charger module
#define STANDBY_PIN 32   // STDBY pin from charger module
...
void updateBattery() {
  ...
  if (systemLocked) {
    ...
    // Update charging status silently
    bool chrgPin = digitalRead(CHARGING_PIN);
    bool stdbyPin = digitalRead(STANDBY_PIN);
    isCharging = (!chrgPin && stdbyPin);
    isFullyCharged = (chrgPin && !stdbyPin);
  ...
  // ===== DETECT CHARGING STATUS FIRST =====
  // Read charging pins (active LOW)
  bool chrgPin = digitalRead(CHARGING_PIN);
  bool stdbyPin = digitalRead(STANDBY_PIN);
  
  // Determine charging state
  bool newIsCharging = (!chrgPin && stdbyPin);      // CHRG=LOW, STDBY=HIGH = Charging
  bool newIsFullyCharged = (chrgPin && !stdbyPin);  // CHRG=HIGH, STDBY=LOW = Fully Charged
  ...
  // Show notification continuously when battery reaches 100% until charger is unplugged
  static bool batteryWasFull = false;
  static bool showingChargedScreen = false;
  static bool chargedScreenDisplayed = false;
  
  // Check if battery just reached 100% while charging
  if (isCharging && bat_percentage >= 100.0 && !batteryWasFull) {
    batteryWasFull = true;
    showingChargedScreen = true;
    chargedScreenDisplayed = false;  // Need to display it
    Serial.println("✅ Battery fully charged to 100% - showing charged screen");
  }
  
  // Display charged screen only once (not every loop iteration)
  if (showingChargedScreen && !chargedScreenDisplayed) {
    display.clearDisplay();
    display.setTextSize(2);
    display.setTextColor(SH110X_WHITE);
    display.setCursor(10, 10);
    ...
    display.println("100%");
    display.display();
    chargedScreenDisplayed = true;  // Mark as displayed
  }
  
  // Check if charger is unplugged (GPIO 27 goes HIGH)
  if (showingChargedScreen) {
    bool chrgPin = digitalRead(CHARGING_PIN);  // GPIO 27
    
    if (chrgPin == HIGH) {
      // Charger unplugged - stop showing charged screen
      showingChargedScreen = false;
      batteryWasFull = false;
      chargedScreenDisplayed = false;
      Serial.println("Charger unplugged - returning to normal screen");
      displayStatusScreen();
    } else {
      // Still showing charged screen - skip rest of battery update
      return;
    }
  }
  
  // Reset flag when battery drops below 99%
  if (bat_percentage < 99.0) {
    batteryWasFull = false;
    if (showingChargedScreen) {
      showingChargedScreen = false;
      chargedScreenDisplayed = false;
...
// Draw compact charging battery icon (CHARGING MODE)
...
void connectWiFi(){
  ...
  display.setCursor(0,30);
  display.println(F("AP: BVS-Scanner-06"));
  ...
  for (int attempt = 1; attempt <= maxAttempts && !credentialsValid; attempt++) {
    ...
    bool portalResult = wm.autoConnect("BVS-Scanner-06", "rob123456");
    
    if (!portalResult) {
      // Portal timeout or user didn't submit - restart
      debugPrint("WiFiManager portal timeout - restarting device...");
      display.clearDisplay();
      displayStatusBar();
      display.setCursor(0, 20);
...
void setup() {
  ...
  // ===== CONFIGURE CHARGING DETECTION PINS =====
  debugPrint("Configuring charging detection pins...");
  pinMode(CHARGING_PIN, INPUT_PULLUP);  // CHRG pin from TP4056/BQ24075
  pinMode(STANDBY_PIN, INPUT_PULLUP);   // STDBY pin from TP4056/BQ24075
  debugPrint("Charging detection pins configured (GPIO 27: CHRG, GPIO 32: STDBY)");
...
void wakeDisplay() {
  ...
  // Directly show "Ready to scan" screen after wake-up
  displayStatusScreen();
}
