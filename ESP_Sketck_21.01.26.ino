...
void updateBattery() {
  ...
  // ===== FULL CHARGE NOTIFICATION (SINGLE CHRG PIN LOGIC) =====
  // Show "BATTERY CHARGED" only when:
  // 1. CHRG pin is HIGH (not charging)
  // 2. Battery percentage is ≥ 99%
  // This indicates charging is complete
  static bool batteryWasFull = false;
  static bool showingChargedScreen = false;
  static bool chargedScreenDisplayed = false;
  
  // Check if charging just finished and battery is full
  // isFullyCharged = (CHRG HIGH && battery ≥ 99%)
  if (isFullyCharged && !isCharging && bat_percentage >= 99.0 && !batteryWasFull) {
    ...
    Serial.println("✅ Charging finished - battery fully charged to 99%+ - showing charged screen");
  }
  ...
  if (showingChargedScreen && !chargedScreenDisplayed) {
    ...
    display.println(String((int)bat_percentage) + "%");
    display.display();
    chargedScreenDisplayed = true;  // Mark as displayed
  }
  
  // Check if charger is unplugged (CHRG pin goes HIGH and battery drops)
  if (showingChargedScreen) {
    bool chrgPin = digitalRead(CHARGING_PIN);  // GPIO 27
    
    if (chrgPin == HIGH && bat_percentage < 99.0) {
      // Charger unplugged and battery dropped - stop showing charged screen
      showingChargedScreen = false;
      batteryWasFull = false;
      chargedScreenDisplayed = false;
      Serial.println("Charger unplugged or battery dropped - returning to normal screen");
      displayStatusScreen();
    } else if (chrgPin == LOW) {
      // Started charging again - stop showing charged screen
      showingChargedScreen = false;
      batteryWasFull = false;
      chargedScreenDisplayed = false;
      Serial.println("Charging started again - returning to normal screen");
      displayStatusScreen();
    } else {
